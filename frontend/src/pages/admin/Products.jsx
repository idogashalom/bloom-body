...
const mockProducts = fallbackProducts.map(p => ({
  ...
  is_available: p.is_available === false || p.is_available === 0 || p.is_available === "0" ? false : Boolean(p.is_available),
  unavailable_message: '',
  is_featured: false,
  image: p.image
}));
...
const Products = () => {
  ...
  return (
    <div>
      <h1 style={{ color: 'var(--deep-pink)', fontSize: '32px', marginBottom: '10px' }}>Products Management</h1>
      ...
      <section style={{ backgroundColor: '#fff', borderRadius: '12px', boxShadow: 'var(--shadow)', padding: '16px', marginBottom: '18px' }}>
        <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ position: 'relative', flex: 1, minWidth: '200px' }}>
            <span style={{ position: 'absolute', left: '12px', top: '10px', color: '#888' }}><i fa fah-search></i></span>
            <input
              type="text"
          ...
          <tbody>
            {products.filter(p => p.name.toLowerCase().includes(searchQuery.toLowerCase())).map(product => (
              <tr key={product.id} style={{ borderBottom: '1px solid #f9f9f9', transition: 'background-color 0.2s' }}>
                <td style={{ padding: '15px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <span style={{ color: 'var(--text-dark)', fontWeight: 'bold', fontSize: '15px' }}>{product.name}</span>
                    <span style={{ color: '#888', fontSize: '12px', marginTop: '4px' }}>SKU: BB00{product.id}</span>
                  </div>
                </td>
                <td style={{ padding: '15px', color: '#555', fontSize: '14px' }}>₦{parseFloat(product.price).toLocaleString()}</td>
                ...
                <td style={{ padding: '15px', textAlign: 'center', whiteSpace: 'nowrap', fontSize: '14px' }}>
